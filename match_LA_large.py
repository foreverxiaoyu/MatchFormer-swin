...
import math
from  model.backbone.swin import SwinTransformerBlock
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, eps=1e-6 , cross = False, flag=False):
        super().__init__()
        ...
        self.flag = flag
        self.swin = SwinTransformerBlock(dim=dim, num_heads=num_heads, window_size=7, shift_size=0, mlp_ratio=4,
    ...
    def forward(self, x):
        ...
        if self.cross == True:
            ...
            # print(f"row match x:{x.shape}")
            x = self.swin(x, N)
            # print(f" outx:{x.shape}\n")
            return x
        else:
            key, value = kv[0], kv[1]
...
class Matchformer_LA_large(nn.Module):
    def __init__(self, img_size=224, in_chans=1, embed_dims=[128, 192, 256, 512], num_heads=[8, 8, 8, 8],
        ...
        self.AttentionBlock1 =  AttentionBlock(img_size=img_size // 2, patch_size=7, num_heads= num_heads[0], mlp_ratios=4,  in_chans=in_chans,
                                              embed_dims=embed_dims[0],stride=2,depths=3, cross=stage1_cross)
